...
public class outPut_Major_path {
	...
	public static void output() throws RowsExceededException, WriteException, IOException{
		output_path=MajorPath.r;
		if(output_path.size()==0) {
			return;
		}
	
	WritableWorkbook writeBook = Workbook.createWorkbook(new File("D:\\ECLIPSE\\workspace\\adhoc\\example\\Initial_major_PATH.xls"));
	for(Map.Entry<Integer, Stack<node>> entry:output_path.entrySet())
	{

		WritableSheet firstSheet = writeBook.createSheet("request path " + entry.getKey(), entry.getKey());
