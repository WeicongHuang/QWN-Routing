...
public class outPut_result {
	static HashMap<Integer, Stack<node>> output_path =new HashMap<Integer, Stack<node>>();
	
	public static void output() throws RowsExceededException, WriteException, IOException{
		output_path=entangle_Swap.link_success_path;
		if(output_path.size()==0) {
			return;
		}
	
	WritableWorkbook writeBook = Workbook.createWorkbook(new File("D:\\ECLIPSE\\workspace\\adhoc\\example\\FinalSuccess_PATH.xls"));
